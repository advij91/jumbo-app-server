  ...
  return (
    <div className="mt-4 bg-gray-50 p-4 rounded-md shadow-inner">
      ...
      <div className="grid grid-cols-2 gap-2 text-sm text-gray-800">
        ...
        <p>
          <span className="font-semibold">Outlet:</span> {order.outletAddress}
        </p>
        <p>
          <span className="font-semibold">Contact:</span> {order.userContact}
        </p>
      </div>
            ...
            {item.addons.length > 0 && (
              <span className="text-gray-500">
                ...
                {item.addons
                  .map(
                    (addon) =>
                      `${addon.category}: ${addon.addonItems
                        .map((ai) => ai.name)
                        .join(", ")}`
                  )
                  .join(" | ")}
      ...
      <div className="flex flex-wrap gap-4 mt-4">
        ...
        {order.orderStatus === "Confirmed" && (
          <button
            ...
            onClick={() => handleStatusUpdate("Ready to Pickup")}
          >
